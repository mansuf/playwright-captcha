...
class SolverTrSolver(ApiSolverBase):
    """Solver.tr external solving service (Turnstile only)."""

    type: SolverType = SolverType.solvertr

    def __init__(self, framework: FrameworkType, page: Page,
                 async_solvertr_client: AsyncSolverTr,
                 max_attempts: int = 3, attempt_delay: int = 5):
        """
        Initialize the Solver.tr solver.

        :param page: Playwright Page object
        :param async_solvertr_client: AsyncSolverTr client instance
        ...
        self.async_solvertr_client = async_solvertr_client
    ...
    async def _solve_captcha_once(
        ...
        """
        Solve captcha using Solver.tr API.
        ...
        # convert captcha_data keys to match Solver.tr API syntax
        param_name_mapping = {
            'site_key': 'sitekey',
        ...
        result = await solver(self.async_solvertr_client, **kwargs)
    ...
    async def get_balance(self) -> float:
        ...
        result = await self.async_solvertr_client.balance()
        return float(result.get('balance', 0.0))

    def get_name(self) -> str:
        return "Solver.tr Solver"
