...
class CapMonsterSolver(ApiSolverBase):
    """CapMonster external solving service."""

    type: SolverType = SolverType.capmonster

    def __init__(self, framework: FrameworkType, page: Page,
                 async_capmonster_client: AsyncCapMonster,
                 max_attempts: int = 3, attempt_delay: int = 5):
        """
        Initialize the CapMonster solver.

        :param page: Playwright Page object
        :param async_capmonster_client: AsyncCapMonster client instance
        ...
        self.async_capmonster_client = async_capmonster_client
    ...
    async def _solve_captcha_once(
        ...
        """
        Solve captcha using CapMonster API.
        ...
        # convert captcha_data keys to match CapMonster API syntax
        param_name_mapping = {
            'site_key': 'sitekey',
        ...
        result = await solver(self.async_capmonster_client, **kwargs)
    ...
    async def get_balance(self) -> float:
        ...
        result = await self.async_capmonster_client.balance()
        return float(result.get('balance', 0.0))

    def get_name(self) -> str:
        return "CapMonster Solver"
